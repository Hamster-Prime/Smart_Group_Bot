...
在 Mini App 群组页逐群配置关键词触发的固定回复，支持「包含 / 完全匹配 / 正则」三种匹配方式。命中后直接发送配置的回复文本并跳过 AI 回复管线（消息仍照常审核、写入记忆）；违规消息与静默名单内的消息不会触发。每条规则可单独选择：是否**置顶**发送的回复、是否纳入**自动删除**（按全局 Bot 设置中的「关键词回复」类别与秒数执行）、是否启用。

### 定时消息（定时置顶）

在 Mini App 群组页逐群配置定时发送的群消息，支持两种节奏：「每天定时」按 HH:MM（Asia/Shanghai）每天发送一次，「固定间隔」按分钟数循环发送（最短 5 分钟）。每条可单独选择：发送后是否**置顶**（即定时置顶）、置顶前是否**取消上一条置顶**、是否纳入**自动删除**（按全局「定时消息」类别执行）、是否启用。调度对比上次发送时间与应发时刻，重启不会漏发或重复发送。

### 入群欢迎语

在 Mini App 群组页逐群配置入群欢迎语，留空则不发送。支持 `{name}`（新成员名称）和 `{mention}`（可点击提及）占位符；文本按纯文本处理并自动转义，不会破坏消息格式。开启入群验证的群，欢迎语在成员**通过验证后**（含管理员直接通过）才发送；未开启验证的群在完成资料筛查后发送。欢迎消息纳入全局「入群欢迎」自动删除类别，可单独设置保留秒数。
...
- Bot 回复、流式输出、上下文、主动话题和按消息类别配置的自动删除策略（每个类别可单独设置删除秒数，留空继承全局秒数）
