...
**成员处置语义**：永久封禁会同时清除该用户在群内的全部历史发言；入群验证超时、资料巡检超时和爆破防护等「仅移出群聊」操作不会封禁，也不会清除历史消息，用户之后仍可重新加入。
...
任何群成员回复骚扰消息并发送 `/voteban [举报理由]`，即对被回复用户发起民主投票；也可在回复目标消息时直接要求 Bot 发起，由 `vote_ban` 技能执行。投票消息带内联「投票封禁」按钮，每人一票（发起人自动投出第一票），达到本群设定票数后立即在本群封禁该用户（同步写入警告/封禁名单，Mini App 可见）。管理员、最高管理员、机器人与匿名身份不可被投票；同一目标同时只能有一个进行中/执行中的投票；超过有效期自动失效。
