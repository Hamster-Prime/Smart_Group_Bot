...
在 Mini App 群组页逐群配置关键词触发的固定回复，支持「包含 / 完全匹配 / 正则」三种匹配方式。回复内容支持换行、可拉伸编辑和安全 Markdown。每条规则还可携带自定义内联按钮（跳转链接、复制文字、分享、管理员删除并可横向排列），按钮可选 Telegram 的 `primary`（蓝）、`success`（绿）、`danger`（红）样式，并单独选择置顶、自动删除和启用状态。
...
任何群成员回复骚扰消息并发送 `/voteban [举报理由]`，即对被回复用户发起民主投票；也可在回复目标消息时直接要求 Bot 发起，由 `vote_ban` 技能执行。投票消息带内联「投票封禁」按钮，每人一票（发起人自动投出第一票），达到本群设定票数后立即在本群封禁该用户、删除最初被投票的消息（同步写入警告/封禁名单，Mini App 可见）。投票消息同时带「取消投票」「直接封禁」两个管理员按钮：群管理员及以上可提前终止投票或跳过计票直接封禁，结果通知与审计账本会记录执行的管理员身份。管理员、最高管理员、机器人与匿名身份不可被投票；同一目标同时只能有一个进行中/执行中的投票；超过有效期自动失效。
