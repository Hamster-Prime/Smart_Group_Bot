...
入群验证按群配置：每个群都能单独关闭、开启或继承全局默认，并可选择 Cloudflare Turnstile、hCaptcha，或「Turnstile + hCaptcha」双重验证。新成员入群后**先于资料筛查立即被禁言**（禁言不等待 LLM 审查结果），需点击群内按钮跳转 bot 私聊，在 Telegram Mini App 内完成该群所选验证码后恢复权限；超时被移出群聊（可重进重试）；管理员在群内提示上点「拒绝」则**直接在本群永久封禁**（与超时移出不同，需管理员手动解封）。验证到达终态（通过 / 拒绝 / 超时 / 封禁）后，群内提示与私聊验证入口消息都会被改写或删除，Mini App 页面也会提前校验并拒绝重复验证。身份由 Telegram initData 签名（HMAC bot token）保证。需要 bot 是带「封禁用户」权限的群管理员（启动时自检并警告）。

**抢跑消息清理**：禁言毕竟晚于入群一瞬，若新成员在禁言生效前抢发了消息，这些消息未经过任何验证——消息入口的验证闸门会直接删除「存在未完成验证记录」的发送者的消息，且验证禁言落地、资料筛查封禁执行时都会追溯删除该成员自本次入群以来抢发的消息（进程内近况缓冲，覆盖典型竞态窗口）。
...
新成员入群时用群规审查其昵称/用户名/简介，命中即自动加入全局封禁名单；此后每条群消息都会复查发送者可见资料（含 `/help`、视频、纯媒体消息），改名无法逃避。全局封禁的用户在任意授权群发言即被删消息并封禁。`/ban`、`/unban` 现在对所有群管理员开放，默认只操作当前群；最高管理员使用时会收到「仅本群 / 全局」内联选择。回复消息执行 `/ban` 时，封禁成功后会同时删除被回复消息；`/spam` 沿用群管理员权限，直接封禁目标、删除被回复的垃圾消息并加入全局封禁名单。`/banlist` 和 Mini App 全局名单仍仅最高管理员可见。

**成员处置语义**：永久封禁和「仅移出群聊」统一直接调用 `banChatMember(revoke_messages=True)`；仅移出随后调用 `unbanChatMember`，不留下封禁记录，用户之后可重新加入。按 Telegram 官方说明，这会撤销被移除账号对旧群历史的访问，但不会删除该账号过去发出的消息；Bot API 目前没有按成员删除全部发言的方法（同名能力只存在于仅用户账号可调用的 MTProto API）。作为补充，bot 在进程内记录每名成员近期消息 id：刚入群的成员被验证禁言或封禁时，其入群后抢发的消息会经 `deleteMessages` 批量追溯删除。
...
短时间批量入群达到阈值时自动锁定群组，锁定期间新加入者会被临时移出但不永久封禁；触发与解除通知都长期保留。全局默认会在防护触发时置顶防护通知，群组可用「继承全局 / 开启 / 关闭」单独覆盖；无论自动锁定超时结束，还是管理员手动结束防护，都会取消该防护通知的置顶。待取消的精确消息 ID 会持久化并自动重试，因此消息替换、短暂网络失败或 Bot 重启不会丢失清理责任。系统会追溯近期加入者并发送真人质询：第一行是被点名成员的验证按钮，第二行是仅管理员可用的「一键移除被追溯用户」。锁定到期后无需等待下一次入群，bot 会主动发送解除通知。
...
在 Mini App 群组页逐群配置关键词触发的固定回复，支持「包含 / 完全匹配 / 正则」三种匹配方式。回复内容支持换行、可拉伸编辑和安全 Markdown。每条规则还可携带自定义内联按钮（跳转链接、复制文字、分享、管理员删除并可横向排列），按钮可选 Telegram 的 `primary`（蓝）、`success`（绿）、`danger`（红）样式，并单独选择置顶、自动删除和启用状态；关键词回复继续使用每条规则自己的置顶开关，不受活动通知的全局默认影响。

### 定时消息（定时置顶）

在 Mini App 群组页逐群配置定时发送的群消息，支持两种节奏：「每天定时」按 HH:MM（Asia/Shanghai）每天发送一次，「固定间隔」按分钟数循环发送（最短 5 分钟）。内容支持换行、可拉伸编辑、安全 Markdown 和与关键词回复相同的自定义内联按钮；同时支持每条任务独立设置置顶、取消上一条置顶、自动删除和启用状态，不受活动通知的全局默认影响。

### 入群欢迎语

在 Mini App 群组页逐群配置入群欢迎语，留空则不发送。支持 `{name}`（新成员名称）和 `{mention}`（可点击提及）占位符、换行、安全 Markdown 与自定义内联按钮；按钮可在行号后追加 `primary`（蓝）、`success`（绿）或 `danger`（红）样式。开启入群验证的群，欢迎语在成员**通过验证后**（含管理员直接通过）才发送；未开启验证的群在完成资料筛查后发送。

### 呼叫管理员（@admin）

群成员发送 `@admin`（或 `@admins`）即可一键呼叫群管理员：机器人发送一条真实 @ 提及管理员的通知（可附带说明文字；回复某条消息发送则同时附上被举报消息内容并锚定回复）。审核不通过的消息不会触发呼叫。全局可配置开关、呼叫冷却时间和通知自动置顶（默认关闭）；Mini App 群组页可逐群覆盖开关，并勾选要 @ 的目标管理员（默认全部勾选，全选状态下新晋管理员自动包含）。开启通知置顶后，管理员可在通知上标记「已处理」并取消置顶。通知消息属于独立的「呼叫管理员」自动删除类别。

### 骚扰民主投票封禁

任何群成员回复骚扰消息并发送 `/voteban [举报理由]`，即对被回复用户发起民主投票；也可在回复目标消息时直接要求 Bot 发起，由 `vote_ban` 技能执行。投票消息带内联「投票封禁」按钮，每人一票（发起人自动投出第一票），达到本群设定票数后立即在本群封禁该用户、删除最初被投票的消息（同步写入警告/封禁名单，Mini App 可见）。投票消息同时带「取消投票」「直接封禁」两个管理员按钮：群管理员及以上可提前终止投票或跳过计票直接封禁，结果通知与审计账本会记录执行的管理员身份。管理员、最高管理员、机器人与匿名身份不可被投票；同一目标同时只能有一个进行中/执行中的投票；超过有效期自动失效。全局默认会在投票进行期间置顶投票消息，群组可用「继承全局 / 开启 / 关闭」单独覆盖；票数达标完成、管理员中途取消、管理员直接封禁或票数不足超时等任一结束路径都会取消置顶。
...
| `routeros_doc` | 实时查询 MikroTik RouterOS 官方手册、CLI 参考与更新记录；配置/脚本问答和审查均先读取官方原文 |
...
| `/spam [用户ID] [原因]` | 封禁垃圾用户并加入全局封禁名单；回复使用时同时删除垃圾消息 |
...
- 审核、Cloudflare Turnstile/hCaptcha、TTS、音乐、影片信息、AV 和贴纸池，以及爆破防护、民主投票、呼叫管理员通知的全局自动置顶默认值
- 日志输出、全部 LLM 提示词，以及每个授权群的独立模型 API、完整默认权限/定时时段、消息模板/按钮、功能开关、活动/通知自动置顶三态覆盖、群规、永久记忆、带用户名的警告/封禁与审核豁免、回复静默名单和说话风格画像
