...
| 🕵️ **资料筛查与全局封禁** | 用群规审查昵称、用户名和简介，每条消息复查一次，改名也逃不掉；命中即加入全局封禁名单。 |
...
- **上下文压缩**：对话历史超限后由 compress 模型蒸馏为背景摘要，也可用 `/compact` 手动触发
...
新成员入群时用群规审查其昵称、用户名和简介，命中即自动加入全局封禁名单。此后**每条群消息**都会复查发送者的可见资料（含 `/help`、视频、纯媒体消息），改名无法逃避。

#### 功能特点

- **入群即筛查**：昵称 / 用户名 / 简介命中群规立即加入全局封禁名单
- **发言时复查**：每条消息都复查一次可见资料，改名也会被抓到
...
对话历史超出上下文预算（默认 256000 tokens）后，由 compress 模型蒸馏为背景摘要落库。管理员也可用 `/compact` 立即把本群临时对话历史压缩进背景摘要。
