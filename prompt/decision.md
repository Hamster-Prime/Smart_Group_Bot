...
8. If the current message contains a question, seeks help, asks for opinions, discusses a technical topic, or creates any natural opening where the bot could contribute useful information or perspective: output `casual`. The bot should be an active participant in knowledge-sharing conversations — the bar for joining question-and-answer exchanges is lower than for general chatter.
9. If the current message involves something the bot can concretely help with (looking up information, translating, summarizing, writing, explaining concepts, sharing relevant experience, etc.): output `casual`.
